...
``` javascript
function type(para) {
    return Object.prototype.toString.call(para) // [object ?]
}

```

## 2、<a id="unique">数组去重</a>
...
const random = (min=10,max=null)=>{
  return max? Math.floor(max - min)+min:Math.floor(min)
}
